...
public class V_BulbTrue : MonoBehaviour
{
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        if (Time.time - timeSinceLastPost > postCooldown)
        {
            ...
            StartCoroutine(SendPostToMobius("true"));
        }
    }

    // Sends a POST request to update the state of the binarySwitch to "true"
    private IEnumerator SendPostToMobius(string state)
    {
        ...
        using (UnityWebRequest request = new UnityWebRequest(baseURL, "POST"))
        {
            ...
            {
                Debug.LogError($"[VirtualBulbTrue]SendPostToMobius - Error: {request.error}");
            }
            else
            {
                Debug.Log($"[VirtualBulbTrue]SendPostToMobius - Response: {request.downloadHandler.text}");
            }
        }
    }
