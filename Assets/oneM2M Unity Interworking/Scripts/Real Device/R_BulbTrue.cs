...
public class RealBulbTrue : MonoBehaviour
{
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        if (Time.time - timeSinceLastPut > putCooldown)
        {
            ...
            StartCoroutine(SendPutToMobius("true"));
        }
    }

    // Sends a PUT request to update the state of the binarySwitch to "true"
    private IEnumerator SendPutToMobius(string state)
    {
        ...
        using (UnityWebRequest request = UnityWebRequest.Put(baseURL, jsonData))
        {
            ...
            {
                Debug.LogError($"[RealBulbTrue]SendPutToMobius - Error: {request.error}");
            }
            else
            {
                Debug.Log($"[RealBulbTrue]SendPutToMobius - Response: {request.downloadHandler.text}");
            }
        }
    }
